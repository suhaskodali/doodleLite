...
class OptionsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create option" do
    assert_difference('Option.count') do
      post options_url, params: { option: { name: @option.name, numVotes: @option.numVotes, poll_id: @option.poll_id } }
    end
  ...
  test "should update option" do
    patch option_url(@option), params: { option: { name: @option.name, numVotes: @option.numVotes, poll_id: @option.poll_id } }
